...
| Field | Your answer |
|-------|-------------|
| **Full name** | Doaa Shaker Mohamed Aziz Awad Elhofy|
| **Email** |doaaelhofy55@gmail.com |
| **Group** | G9 |
...
*(If R3 is moved before R1, the Garage lamp will end up OFF during an overheat emergency, leaving a dangerously hot room in the dark. 

This happens because the rules execute sequentially from top to bottom, meaning the last rule to modify the room's status byte gets the final say. If R3 runs first, it correctly sees the 46°C temperature and turns the lamp ON. However, a split second later, R1 runs, sees that the room is empty, and turns the lamp OFF. Because the convenience rule (R1) runs last, it completely overwrites the safety rule (R3). To ensure emergency protocols cannot be undone by basic occupancy checks, R3 must always be placed after R1.)*
